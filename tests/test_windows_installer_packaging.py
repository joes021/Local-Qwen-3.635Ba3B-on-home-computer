...
class WindowsInstallerPackagingTests(unittest.TestCase):
    ...
    def test_packaging_and_shortcuts_include_uninstall_entrypoints(self):
        ...
        start_control_center_next_content = (WINDOWS_LAUNCHER_DIR / "start-control-center-next.ps1").read_text(encoding="utf-8")
        ...
        self.assertIn("Get-ControlCenterNextCandidateRoots", start_control_center_next_content)
        self.assertIn("local-qwen-control-center-next", start_control_center_next_content)
        self.assertIn("control-center.ps1", start_control_center_next_content)
        self.assertIn('New-HiddenVbsLauncher -LaunchersDir $launchersDir -VbsName "open-control-center.vbs" -PsScriptName "start-control-center-next.ps1"', common_content)
    ...
    def test_linux_install_update_downloads_versioned_release_asset(self):
        ...
        self.assertIn('RELEASE_URL="$(python3 - <<\'PY\' "$LATEST_JSON"', content)
        self.assertIn('TARGET_PATH="$TARGET_DIR/Local-Qwen-Setup-$LATEST_VERSION.run"', content)
        self.assertIn('echo "Trenutna verzija: v$CURRENT_VERSION"', content)
        self.assertIn('echo "Latest javna verzija: v$LATEST_VERSION"', content)
        self.assertIn('echo "Nova verzija: v$LATEST_VERSION"', content)
        self.assertIn('echo "Release URL: $RELEASE_URL"', content)
    ...
    def test_linux_check_updates_reads_version_json_with_bom_tolerance(self):
        ...
        self.assertIn('encoding="utf-8-sig"', content)
        self.assertIn('if [[ "${1:-}" == "--json" ]]; then', content)
        self.assertIn('LATEST_JSON="$(run_runtime_engine_json latest-release', content)
    ...
    def test_linux_manage_models_hides_internal_or_conflicting_badges_in_cli_output(self):
        ...
        self.assertIn('add_unsloth_custom_model()', content)
        self.assertIn('customSource": "unsloth"', content)
        self.assertIn('add-unsloth <repo> <filename>', content)
    ...
    def test_linux_run_package_prefers_gui_wizard_but_keeps_tui_fallback(self):
        ...
        start_opencode_script = (REPO_ROOT / "launcher" / "linux" / "start-opencode.sh").read_text(encoding="utf-8")
        control_center_script = (REPO_ROOT / "launcher" / "linux" / "control-center.sh").read_text(encoding="utf-8")
        dashboard_script = (REPO_ROOT / "launcher" / "linux" / "control-center-dashboard.sh").read_text(encoding="utf-8")
        actions_script = (REPO_ROOT / "launcher" / "linux" / "control-center-actions.sh").read_text(encoding="utf-8")
        desktop_launch_script = (REPO_ROOT / "launcher" / "linux" / "desktop-launch.sh").read_text(encoding="utf-8")
        ...
        self.assertIn('TURBOQUANT_CONFIG_PATH="$(get_turboquant_config_path)"', start_server_script)
        self.assertIn('runtime_preference = str(turboquant_config.get("runtimePreference") or "turboquant").strip().lower()', start_server_script)
        self.assertIn('cache_type_k = str(turboquant_config.get("ctk") or profile_data["cacheTypeK"])', start_server_script)
        self.assertIn('cache_type_v = str(turboquant_config.get("ctv") or profile_data["cacheTypeV"])', start_server_script)
        self.assertIn('mmap_mode = str(turboquant_config.get("mmapMode")', start_server_script)
        self.assertIn('flash_attention = bool(turboquant_config.get("flashAttention", True))', start_server_script)
        self.assertIn('mlock = bool(turboquant_config.get("mlock", state.get("mlock", True)))', start_server_script)
        self.assertIn('get_turboquant_config_path()', (REPO_ROOT / "launcher" / "linux" / "local_qwen_common.sh").read_text(encoding="utf-8"))
        self.assertIn('resolve_opencode_command()', (REPO_ROOT / "launcher" / "linux" / "local_qwen_common.sh").read_text(encoding="utf-8"))
        self.assertIn('is_windows_interop_path()', (REPO_ROOT / "launcher" / "linux" / "local_qwen_common.sh").read_text(encoding="utf-8"))
        self.assertIn('test_llama_process_running()', (REPO_ROOT / "launcher" / "linux" / "local_qwen_common.sh").read_text(encoding="utf-8"))
        self.assertIn('Sacuvani lifecycle je tvrdio da je server aktivan', (REPO_ROOT / "launcher" / "linux" / "local_qwen_common.sh").read_text(encoding="utf-8"))
        self.assertIn('Server je ostao u starting stanju bez health endpoint-a i bez llama-server procesa.', (REPO_ROOT / "launcher" / "linux" / "local_qwen_common.sh").read_text(encoding="utf-8"))
        self.assertIn('Pokusavam automatsku Linux instalaciju OpenCode-a u \\$HOME/.local ...', start_opencode_script)
        self.assertIn('npm install -g opencode-ai --prefix "$HOME/.local"', start_opencode_script)
        self.assertIn('OpenCode executable: $OPENCODE_CMD', start_opencode_script)
        self.assertIn('resolve_opencode_command', start_opencode_script)
        self.assertIn('Pokrecem OpenCode u ovom terminalu...', start_opencode_script)
        self.assertIn('exec "$SCRIPT_DIR/control-center-dashboard.sh"', control_center_script)
        self.assertIn('control_center_has_tui', actions_script)
        self.assertIn('whiptail --title "$title" --menu', actions_script)
        self.assertIn('show_home_menu()', dashboard_script)
        self.assertIn('run_dashboard_menu "Home"', dashboard_script)
        self.assertIn('run_dashboard_menu "Pokretanje"', dashboard_script)
        self.assertIn('find_terminal_launcher()', desktop_launch_script)
        self.assertIn('x-terminal-emulator', desktop_launch_script)
        self.assertIn('nohup "$terminal" -e bash -lc "$inner_command"', desktop_launch_script)
        self.assertIn('gnome-terminal', desktop_launch_script)
        self.assertIn('konsole', desktop_launch_script)
        self.assertIn('Exec=$LAUNCHERS_DIR/desktop-launch.sh $LAUNCHERS_DIR/control-center.sh', install_script)
        self.assertIn('Exec=$LAUNCHERS_DIR/desktop-launch.sh $LAUNCHERS_DIR/start-opencode.sh balanced', install_script)
        self.assertIn('Terminal=false', install_script)
        ...
        self.assertIn('selected_model = None', (REPO_ROOT / "launcher" / "linux" / "configure-settings.sh").read_text(encoding="utf-8"))
        self.assertIn('config["small_model"] = f"local-llamacpp/{state[\'modelId\']}"', (REPO_ROOT / "launcher" / "linux" / "configure-settings.sh").read_text(encoding="utf-8"))
        self.assertIn('if normalized_label.lower().endswith(" local"):', (REPO_ROOT / "launcher" / "linux" / "configure-settings.sh").read_text(encoding="utf-8"))
        self.assertIn('display_name = f"{normalized_label} (llama.cpp)"', (REPO_ROOT / "launcher" / "linux" / "configure-settings.sh").read_text(encoding="utf-8"))
        self.assertIn('display_name = f"{normalized_label} Local (llama.cpp)"', (REPO_ROOT / "launcher" / "linux" / "configure-settings.sh").read_text(encoding="utf-8"))
        self.assertIn('print(f"Settings: {settings_path}")', (REPO_ROOT / "launcher" / "linux" / "configure-settings.sh").read_text(encoding="utf-8"))
        self.assertIn('print(f"OpenCode config: {opencode_path}")', (REPO_ROOT / "launcher" / "linux" / "configure-settings.sh").read_text(encoding="utf-8"))
        self.assertIn('install_script="$(get_local_qwen_install_script_path || true)"', (REPO_ROOT / "launcher" / "linux" / "manage-models.sh").read_text(encoding="utf-8"))
        self.assertIn('MODEL_ID="${MODEL_ID:-}" \\', (REPO_ROOT / "launcher" / "linux" / "manage-models.sh").read_text(encoding="utf-8"))
        ...
        self.assertIn('MODEL_DOWNLOAD_VENV_DIR="$ROOT/state/model-download-venv"', (REPO_ROOT / "launcher" / "linux" / "manage-models.sh").read_text(encoding="utf-8"))
        self.assertIn('ensure_model_download_python()', (REPO_ROOT / "launcher" / "linux" / "manage-models.sh").read_text(encoding="utf-8"))
        self.assertIn('Add-UnslothCustomModel', (REPO_ROOT / "launcher" / "windows" / "local-qwen-common.ps1").read_text(encoding="utf-8"))
        self.assertIn('customSource = "unsloth"', (REPO_ROOT / "launcher" / "windows" / "local-qwen-common.ps1").read_text(encoding="utf-8"))
        self.assertIn('Get-ControlCenterNextTurboQuantConfigPath', (REPO_ROOT / "launcher" / "windows" / "local-qwen-common.ps1").read_text(encoding="utf-8"))
        self.assertIn('$turboQuantConfigPath = Get-ControlCenterNextTurboQuantConfigPath', (REPO_ROOT / "launcher" / "windows" / "start-server.ps1").read_text(encoding="utf-8"))
        self.assertIn('$runtimePreference = if ($turboQuantConfig -and $turboQuantConfig.runtimePreference)', (REPO_ROOT / "launcher" / "windows" / "start-server.ps1").read_text(encoding="utf-8"))
        self.assertIn('$ncmoe = if ($turboQuantConfig -and $turboQuantConfig.ncmoe)', (REPO_ROOT / "launcher" / "windows" / "start-server.ps1").read_text(encoding="utf-8"))
        self.assertIn('$cacheTypeK = if ($turboQuantConfig -and $turboQuantConfig.ctk)', (REPO_ROOT / "launcher" / "windows" / "start-server.ps1").read_text(encoding="utf-8"))
        self.assertIn('$cacheTypeV = if ($turboQuantConfig -and $turboQuantConfig.ctv)', (REPO_ROOT / "launcher" / "windows" / "start-server.ps1").read_text(encoding="utf-8"))
        ...
        self.assertIn('if line.startswith("Settings: ") and line.endswith("/state/settings.json"):', repair_model_linux)
        self.assertIn('if line.startswith("OpenCode config: ") and line.endswith("/.config/opencode/opencode.json"):', repair_model_linux)
